...
// ── Credentials store ─────────────────────────────────────────────────────────
// Secrets (and the non-secret org_id/dept_id config) live here, keyed by short
// account alias, so the agent references an account by name (--account <alias>)
// instead of pasting long tokens. See the committed .zoho-credentials.json for
// the example shape:
//   { "accounts": { "<alias>": { label, client_id, client_secret, refresh_token, org_id, dept_id } }, "default": ... }
...
// Extract --account/-a from argv at load (before entry scripts parse their args
// with strict parsers), storing the value and removing it from argv.
...
// Resolved-account field with env fallback (for non-secret config like org/dept ids).
...
function requireCredentials() {
  ...
  if (ZOHO_CLIENT_ID && ZOHO_CLIENT_SECRET && ZOHO_DESK_REFRESH_TOKEN) {
    return { client_id: ZOHO_CLIENT_ID, client_secret: ZOHO_CLIENT_SECRET, refresh_token: ZOHO_DESK_REFRESH_TOKEN }
  }
...
export function requireOrgId() {
  const id = accountField('org_id', 'ZOHO_DESK_ORG_ID')
  if (!id) { console.error('No org_id set. Add it with: node creds.mjs set <alias> --org-id <id> (or ZOHO_DESK_ORG_ID env)'); process.exit(EXIT.AUTH) }
  return id
}
...
export function manageCreds(argv) {
  ...
  const plainFields = ['label', 'org_id', 'dept_id']
  const view = (e) => ({
    label: e.label ?? null, org_id: e.org_id ?? null, dept_id: e.dept_id ?? null,
  ...
  if (sub === 'set') {
    const alias = argv[1]
    if (!alias) { console.error('Usage: creds.mjs set <alias> [--label L] [--client-id ID] [--client-secret S] [--refresh-token T] [--org-id ID] [--dept-id ID] [--default] [--stdin]'); process.exit(EXIT.USAGE) }
    const entry = store.accounts[alias] ?? {}
    let useStdin = false, makeDefault = false
    const flag = { '--label': 'label', '--client-id': 'client_id', '--client-secret': 'client_secret', '--refresh-token': 'refresh_token', '--org-id': 'org_id', '--dept-id': 'dept_id' }
